...
class AgentClient extends BaseClient {
  ...
  async chatCompletion({ payload, userMCPAuthMap, abortController = null }) {
    ...
    try {
      ...
      const isEmptyResponseError = typeof err?.message === 'string'
        ? err.message.includes("Cannot read properties of null (reading '0')")
        : false;

      if (isEmptyResponseError) {
        logger.warn('[sendCompletion] Empty or invalid response from model');
