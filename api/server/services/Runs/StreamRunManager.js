...
class StreamRunManager {
    ...
    const deltaHandler = async (delta) => {
      for (const key in delta) {
        ...
        if (Array.isArray(delta[key])) {
          ...
          // Concatenate strings for streaming tool call arguments
          data[key] += delta[key];
          ...
          // (e.g., ModelScope sends null as the final delta chunk)
          logger.debug('[StreamRunManager] Ignoring null delta for accumulated string:', {
            key,
  ...
  handleNewToolCall(stepId, toolCall) {
    ...
    // Debug: Log new tool call from ModelScope API
    const toolCallData = toolCall[toolCall.type];
    logger.debug('[StreamRunManager] New tool call received:', {
      stepId,
      toolCallId: toolCall.id,
      toolName: toolCallData?.name,
      toolType: toolCall.type,
      initialArgs: toolCallData?.arguments || toolCallData?.input || 'N/A',
  ...
  async onRunRequiresAction(event) {
    ...
    // Debug: Log tool calls requiring action from ModelScope API
    logger.debug('[StreamRunManager] Tool calls requiring action:', {
      toolCallsCount: submit_tool_outputs.tool_calls.length,
      toolCalls: submit_tool_outputs.tool_calls.map((item) => ({
        id: item.id,
        functionName: item.function?.name,
        rawArguments: item.function?.arguments,
        argumentsType: typeof item.function?.arguments,
        argumentsIsEmpty: item.function?.arguments === '' || item.function?.arguments === null || item.function?.arguments === undefined,
      })),
    });
    ...
    const actions = submit_tool_outputs.tool_calls.map((item) => {
      ...
      try {
        ...
        logger.debug('[StreamRunManager] Successfully parsed tool call arguments:', {
          toolName: functionCall.name,
          toolCallId: item.id,
          parsedArgs: args,
        });
      } catch (e) {
        logger.error('[StreamRunManager] Failed to parse tool call arguments:', {
          toolName: functionCall.name,
          toolCallId: item.id,
