  ...
  for (const message of payload) {
    ...
    for (const part of message.content) {
      if (part.type === ContentTypes.TEXT && part.tool_call_ids) {
        ...
        // Debug: Log raw args from ModelScope API response
        logger.debug('[formatAgentMessages] Raw tool call args:', {
          toolName: tool_call.name,
          toolId: tool_call.id,
          rawArgs: _args,
          argsType: typeof _args,
          argsIsEmpty: _args === '' || _args === null || _args === undefined,
        });
        ...
        try {
          ...
          logger.debug('[formatAgentMessages] Successfully parsed args as JSON:', {
            toolName: tool_call.name,
            parsedArgs: args,
          });
        } catch (e) {
          if (typeof _args === 'string') {
            args = { input: _args };
            logger.debug('[formatAgentMessages] Failed to parse args, wrapped in input object:', {
              toolName: tool_call.name,
              error: e.message,
              wrappedArgs: args,
            });
          } else {
            logger.debug('[formatAgentMessages] Args is not a string, using as-is:', {
              toolName: tool_call.name,
              args: args,
            });
          }
