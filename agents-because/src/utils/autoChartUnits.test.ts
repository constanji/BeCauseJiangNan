...
describe('normalizeAutoChartUnits', () => {
  it('converts majority large 万元 chart values to 亿元', () => {
    ...
    expect((charts[0].data as Array<Record<string, number>>)[0].value).toBe(4986.05);
    expect((charts[0].data as Array<Record<string, number>>)[1].value).toBe(4904.11);
  });
  ...
  it('converts the default multi-organization pie values to two decimal 亿元', () => {
    ...
    expect((chart.data as Array<Record<string, number>>).map((row) => row.index_value)).toEqual([
      455.86,
      785.21,
    ]);
  });
  ...
  it('does not mutate source rows', () => {
    ...
    normalizeAutoChartUnits(
      [{ data: [{ value: 20000 }], xField: 'x', yFields: ['value'], unit: '万元' }],
      rows,
    );
  ...
  it('converts legacy echartsOption series and axis unit', () => {
      ...
      [
        {
          echartsOption: {
            ...
            series: [{ type: 'line', data: [49860525, { name: '上月末', value: 49041141 }] }],
          },
        },
      ],
      [{ mea_unit: '万元' }],
    ...
    const data = ((option.series as Array<Record<string, unknown>>)[0].data) as unknown[];
