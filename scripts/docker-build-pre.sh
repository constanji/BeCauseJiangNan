...
if [ -d "agents-because/dist" ] && [ -n "$(ls -A agents-because/dist 2>/dev/null)" ]; then
  echo "[1/4] agents-because: 发现本地 dist，将复用（若需最新请先删除 agents-because/dist）"
else
  echo "[1/4] agents-because: 无本地 dist，Docker 内将自动构建"
fi

# 2. packages/data-provider（必须：client 构建依赖其 dist，否则新 API 会报 is not a function）
echo "[2/4] 构建 packages/data-provider..."
npm run build:data-provider

# 3. packages/client（必须：Docker 直接 COPY，不构建）
echo "[3/4] 构建 packages/client..."
npm run build:client-package

# 4. client（必须：Docker 直接 COPY，不构建）
echo "[4/4] 构建 client..."
