...
import { parseEChartsToolOutput } from './EChartsMarkers';
...
function isAutoGeneratedEChartsArgs(args: string | Record<string, unknown> | undefined): boolean {
  if (args == null) {
    return false;
  }
  try {
    const parsed = typeof args === 'string' ? JSON.parse(args) : args;
    return (parsed as { _autoGenerated?: unknown })?._autoGenerated === true;
  } catch {
    return false;
  }
}
...
export default function ToolCall({
  ...
  const autoECharts = useMemo(() => {
    ...
    if (!isAutoGeneratedEChartsArgs(_args)) {
      return null;
    }
  ...
  useEffect(() => {
    if ((hasChartOutput || (autoECharts != null && autoECharts.length > 0)) && hasInfo) {
      setShowInfo(true);
    }
  }, [hasChartOutput, autoECharts, hasInfo]);
