...
function isAutoGeneratedEChartsArgs(input: string): boolean {
  if (!input) {
    return false;
  }
  try {
    const parsed = JSON.parse(input) as { _autoGenerated?: unknown };
    return parsed?._autoGenerated === true;
  } catch {
    return false;
  }
}
...
export default function ToolCallInfo({
  ...
  /** Auto charts render in ToolCall.tsx (always visible); hide raw JSON here */
  const hideRawOutputForAutoChart =
    function_name === 'echarts_generator_app' && isAutoGeneratedEChartsArgs(input);
  ...
  return (
    <div className="w-full p-2">
      <div style={{ opacity: 1 }}>
        ...
        {output && !chartData && !hideRawOutputForAutoChart && (
          <>
