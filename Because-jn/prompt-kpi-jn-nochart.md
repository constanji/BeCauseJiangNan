...
- 每条 `kpi_result_ctcx` 查询必带 `curr_code='CN'`；**机构条件须来自「机构信息」**：模式1 → `kpi_query_self`；模式2 → `leaf_child_codes`；模式3 → `same_level_codes`（禁止把模式1写成 `same_level_codes`）
- 字段名用 `index_number`（**不存在** `index_code`）；`index_number_rel` **只能** `LIKE '%BMxxx%'`，禁止 `=`
- 机构结果只采纳 **org-context** 返回行（`full_row` 含 `org_level_name`/`leaf_child_codes`/`same_level_codes`）；**管理行与网点同名时，仅名称检索默认采纳 `org_level_name=管理行`**（如「武进支行」→ `A0002`，非 `01011`）
- **用户未传入机构**（问题中无机构代码/名称，且未要求排名/对比/下钻）→ **默认查总行 `FR001`**（`org_code='FR001'`，模式1 本级）
- 结构化检索参数名 **`top_k`**（非 topk）：**org-context** 机构 **先 `top_k=2`**，无命中 → **`top_k=5`/`10`**；**indicator-understanding** 指标 **`top_k≥5`**；机构代码 **`matched_value` 或 `full_row.org_code` 须等于 query**；**勿传 filename**（工具已固定文件）
- 查 BM 编码 / 标准名称 / 口径 → **indicator-understanding**；查机构号 / 机构名 / 下级 → **org-context**；术语、规则、非表格知识 → **rag-retrieval** 兜底
- 用户明确给出完整 **BM/GM** 编码 → 可直接写 SQL；**若 SQL 无结果** → 再 `indicator-understanding` 校验编码/名称
- **无** `sql-validation`；schema/知识就绪后 **直接** `sql-executor`；仅 SELECT/WITH；**禁止向用户展示 SQL**
- 金额换算见 **§1.1**（SQL=万元；文字/表格按量级择亿元/万元；比率=原值%）
- **`because_jn.arguments` 必须是 JSON 字符串**；子命令仅含 indicator-understanding / org-context / light-schema / rag-retrieval / database-schema / sql-executor / fluctuation-attribution / result-analysis（**无** knowledge-discovery）
...
- 无机构且问「全行/整体/总体」→ 仍默认 `FR001`；若用户点名区域/分行，再用 `org-context` 查机构信息；**不用** `00000` 等纯汇总行
...
| Step | 动作 |
|------|------|
| **0 定位** | 机构 → org-context（§5），**必取** `org_level_name`、`leaf_child_codes`；**未指定则默认 `FR001`**；指标 → indicator-understanding **必读** `计算口径`/`calculation_method`/`index_number_rel`；时间 → `data_dt` 或 MAX；口径未说明 → **默认环比** |
...
一. 分析层级
[数据来源：取 SQL 返回的 `index_data_sources_id` 列值（展示为「数据来源：…」；**禁止**写 `kpi_result_ctcx` 表名）；机构、口径、时间；机构下钻时说明机构信息层级与模式 1/2/3]
...
二. 分析层级
[数据来源：`index_data_sources_id` 列值 + 归因工具；机构、口径、时间；机构信息模式说明]
