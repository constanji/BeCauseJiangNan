...
class FluctuationAttributionTool extends Tool {
  ...
  description =
    ...
    '默认返回 LLM 消费版摘要（compact=true）：overview / top_contributors / conclusion 等精简字段；' +
    'comprehensive 默认不算 metric_attribution；调试或需要完整明细时传 compact:false 或 verbose:true。';
  ...
  schema = z.object({
    ...
    compact: z
      ...
      .default(true)
      .describe(
        '是否返回 LLM 消费版摘要（默认 true）：overview / top_dimension / top_contributors 等；' +
        '传 false 或 verbose:true 时返回接近 2.0 的完整报告（仍无缩进 JSON）。',
      ),
    verbose: z
      .boolean()
      .optional()
      .default(false)
      .describe('调试开关：true 时等同 compact:false，返回完整归因报告'),
    include_metric_attribution: z
      .boolean()
      .optional()
      .default(false)
      .describe(
        '是否在 comprehensive 模式下额外计算 metric_attribution（默认 false；' +
        'analysis_type=metric 时始终计算）',
      ),
  });
  ...
  async _call(input) {
    try {
      logger.info('[FluctuationAttributionTool] 开始波动归因分析:', {
        ...
        compact: input.verbose !== true && input.compact !== false,
      ...
      const compact = input.verbose !== true && input.compact !== false;
      ...
      // 指标归因：comprehensive 默认不算，仅 metric 或显式 include_metric_attribution 时计算
      const shouldComputeMetricAttribution =
        input.analysis_type === 'metric' || input.include_metric_attribution === true;

      if (shouldComputeMetricAttribution) {
      ...
      // 生成后续建议（含 sql_hint，compact 模式下仅投影为 drill_query_hint）
      result.next_steps = this._generateNextSteps(result, input);

      const outputPayload = compact
        ? this._buildLlmSummary(result, input)
        : result;
  ...
  /**
   * 3.0 默认输出：LLM 消费版摘要（不返回完整 next_steps / dimensionRanking 等）
   */
  _buildLlmSummary(result, input) {
    const overview =
      result.time_comparison?.overview ||
      result.dimension_attribution?.overview ||
      null;

    const topDim = result.dimension_attribution?.dimensionRanking?.[0];
    let top_dimension = null;
    let top_contributors = [];

    if (topDim) {
      top_dimension = {
        dimension: topDim.dimension,
        explanatoryPower: topDim.explanatoryPower,
    ...
    const drill_query_hint = firstStep
      ? firstStep.filter || firstStep.sql_hint || firstStep.action || null
      : null;
